...
function checkPage(tabId, tabUrl) {
  ...
  chrome.tabs.executeScript(tabId, { code: "document.body.innerText" }, function (result) {
    const bodyText = result[0].toLowerCase();
    if (bodyText.includes("chatgpt is at capacity right now")) {
      refreshCount++;
      chrome.browserAction.setBadgeText({ text: refreshCount.toString() });
      if (refreshCount === 1) {
